...
xui.head = function() {
        ...
        return function() {
            if (head == undefined) {
                head = x$(document.getElementsByTagName("head")[0]);
            }
...
*/
	...
	callback: function(callback, scope, args, delay){
        if(xui.isFunction(callback)){
            args = args || [];
            scope = scope || window;
            if (delay) {
                xui.defer(callback, delay, scope, args);
            } else {
...
xui.view = {
	...
	switchPart: function(partId, viewName) {
		xui.loadmask.mask("<div style='margin-left:auto;margin-right:auto;'>loading...</div>");
		this._showView(x$(partId),x$(viewName).innerHTML);
		xui.loadmask.unmask();
	},
	
	switchView: function(partId, viewUrl) {
		xui.loadmask.mask("<div style='margin-left:auto;margin-right:auto;'>loading...</div>");
 		x$(partId).xhr(viewUrl,{method:'get',async:false,callback:function(){
			//alert(this.responseText);
			xui.view._showView(partId, this.responseText);
				}});
		
	},
	
	_showView: function(partId, viewData) {
...
xui.jsonp = {
     ...
     */
    request: function(options1){
       options = xui.apply({}, options);

        //<debug>
        if (!options.url) {
            alert('A url must be specified for a JSONP request.');
        }
        //</debug>

        var me = this,
            disableCaching = xui.isDefined(options.disableCaching) ? options.disableCaching : me.disableCaching,
            cacheParam = options.disableCachingParam || me.disableCachingParam,
            id = ++me.statics().requestCount,
            callbackName = options.callbackName || 'callback' + id,
            callbackKey = options.callbackKey || me.callbackKey,
            timeout = xui.isDefined(options.timeout) ? options.timeout : me.timeout,
            params =options.params,
            url = options.url,
            request,
            script;

        params[callbackKey] = 'Jsonp.' + callbackName;
        if (disableCaching) {
        ...
        me[callbackName] = xui.bind(me.handleResponse, me, [request], true);
        xui.head.appendChild(script);
        return request;
    },
    ...
    setupErrorHandling: function(request){
        request.script.onerror = xui.bind(this.handleError, this, [request]);
    },
    ...
    handleResponse: function(result, request){
        ...
        x$(request.script).remove();

        if (request.errorType) {
            success = false;
            xui.callback(request.failure, request.scope, [request.errorType]);
        } else {
            xui.callback(request.success, request.scope, [result]);
        }
        xui.callback(request.callback, request.scope, [success, result, request.errorType]);
    },
