...
public class CommonDao extends HibernateDaoSupport implements ICommonDao {
	...
	public List find(String countString, String queryString, Object args[],
			Pager pager) {
	...
	public List findToMapRow(String countString, String queryString,
			Object args[], Pager pager) {
		int count = count(countString, args).intValue();
		pager.setReCount(count);
		pager.account();
		if (count == 0)
			...
			return findToMapRow(queryString, args, pager.getPageSize(), pager
					.getPage());
	}
		...
		return (Integer) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
						...
						String newQueryString = (new StringBuilder(
								"select count(*) ")).append(queryString)
								.toString();
	...
	public List find(final String queryString, final Object args[],
			final int pageCount, final int pageNum) {
	...
	public List findToMapRow(final String queryString, final Object args[],
			final int pageCount, final int pageNum) {
		return getHibernateTemplate().executeFind(new HibernateCallback() {
		...
		if (object instanceof IBeanPrimaryKey) {
			try {
				if (((IBeanPrimaryKey) object).getId() == null
					...
					((IBeanPrimaryKey) object).setId(Long
							.valueOf(SequenceManager.nextID(100)));
				}
	...
	public List findBySql(final String queryString, final Object args[],
			final int pageCount, final int pageNum) {
		return getHibernateTemplate().executeFind(new HibernateCallback() {
		...
		return (Integer) getHibernateTemplate().execute(
				new HibernateCallback() {
					...
					public Object doInHibernate(Session session)
							throws HibernateException {
						String newQueryString = (new StringBuilder(
								"select count(*) ")).append(countString)
								.toString();
						SQLQuery query = session.createSQLQuery(newQueryString);
						for (int i = 0; args != null && i < args.length; i++)
							query.setParameter(i, args[i]);

						return new Integer(query.list().iterator().next()
								.toString());
					}
	...
	public List findBySql(String countString, String queryString,
			Object args[], Pager pager) {
		...
		if (count == 0)
			...
			return findBySql(queryString, args, pager.getPageSize(), pager
					.getPage());
	}
	...
	public List find(String countString, String queryString, Object args[],
		...
		if (count == 0)
			...
			return find(queryString, args, pageInfo.getPageSize(), pageInfo
					.getPageNum());
