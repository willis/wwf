...
hovertipShowUnderMouse = function(el) {
  ...
  if (!hovertipIsVisible(el)) {
  	...
  	if(navigator.userAgent.indexOf("MSIE")>0) { 
        maxWidth = document.getElementById("bigTable").clientWidth - 50;
  	} 
  	if(isFirefox=navigator.userAgent.indexOf("Firefox")>0){ 
        maxWidth = document.getElementById("bigTable").clientWidth - 100;
   	} 
  	
  	
  	if(hovertipMouseX + 400 > maxWidth) {
  		el.ht.showing = // keep reference to timer
      	window.setTimeout(function() {
          el.ht.tip.css({
              'position':'absolute',
                'top': hovertipMouseY + 'px',
                'right': maxWidth-hovertipMouseX + 'px'})
            .show();
        }, el.ht.config.showDelay);
  	}else if(hovertipMouseX + 400 < maxWidth) {
  		el.ht.showing = // keep reference to timer
      	window.setTimeout(function() {
          el.ht.tip.css({
              'position':'absolute',
