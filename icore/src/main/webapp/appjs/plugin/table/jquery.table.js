/*******************************************************************************
 * 简单表格v1.0 author:tianbao var myTable1 = new MaxTable(); myTable1.initialize( {
 * table:'myTable', loading:'loading', id:'id',
 * queryUrl:'sysUserAction.do?method=index',
 * headerColumns:[{id:'id',name:'操作',renderer:IdCheckBoxRenderer},
 * {id:'id',name:'操作',renderer:editRenderer}, {id:'username',name:'用户名'}, ] } )
 * function query(){ myTable1.page.totalRowNum = 0;
...
MaxTable.prototype = {
					...
					success : function(datas) {
						...
						if (data.length == 0) {
							...
							if (_parent.page.totalRowNum != 0) {
								...
								var tdObj = trObj.insertCell(0);
								tdObj.colSpan = opts.headerColumns.length;
								tdObj.innerHTML = _parent
								.createPage(datas.pageInfo);
	...
	getQueryData : function(parameters) {
		...
		// alert(param_str);
		var jsonData = {
			_gt_json : "{pageInfo:" + $.toJSON(this.page)
					+ ",action:'load',filterInfo:[],sortInfo:["
					+ (this.sortInfo == null ? '' : $.toJSON(this.sortInfo))
					+ "]}"
		};
