...
<html>
	<head>
		...
		<script>
		var login_result = ${empty  userOBJ?'true':'false'};
		if(login_result){
			window.location = '${cxp}/login.jsp';
		}
		
			var isHiddenHead = false;
			function yincangHead(){
				if(isHiddenHead){
					isHiddenHead = false;
					$('#header').show();$('#loginInfo').show();
					$('#yincang').attr('style','position: absolute; top: 35px; right: 0px; width: 50px;');
					$('#yincang a').attr('style','color:#000');
				}else{
					isHiddenHead = true;
					$('#header').hide();$('#loginInfo').hide();
					$('#yincang').attr('style','position: absolute; top: 10px; right: 0px; width: 50px;');
					$('#yincang a').attr('style','color:#fff');
				}
	...
	<body style="height: 100%;">
		<div id="yincang" style="position: absolute; top: 35px; right: 0px; width: 50px;">
			<a href="javascript:void(0)" style="color:#000" onClick="yincangHead()">隐藏</a>
		</div>
				...
				<td>
					...
					<div id="loginInfo">
						...
						<div id="loginInfo_content">
							<div id="webjx" style="float: both;"></div>
							<script>
