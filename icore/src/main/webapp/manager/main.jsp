	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0"
	  ...
	  setInterval("document.getElementById('webjx').innerHTML=new Date().toLocaleString()+' 星期'+'日一二三四五六'.charAt(new      Date().getDay());",1000);
					...
					<ul>
						...
						<pager:eachMenu var="menu" alias="menu_system" levelLimit="1"
							...
							<s:if test="#attr.index>0">
								<li><a
									href="${cxp}/manager/main.jsp?menuAlias=${menu.alias}"
									${param.menuAlias== menu.alias ? 'class=active' : '' } >${menu.name}</a>
								</li>
							</s:if>
						</pager:eachMenu>
					</ul>
		...
		<tr>
			...
			<td>
				<!-- tree begin管理 --> <pager:menu var="sysMenuBean"
					alias="${param.menuAlias}" check="true" />
				<div id="content">
					<a href="#" id="allow2"></a>
					<div id="left" style="overflow: auto">
						<a href="#" id="allow"></a>

						<h2 class="person">${sysMenuBean.name}</h2>
						<span class="userName"> <!--  --> <FONT
							title="${userOBJ.truename}"> ${userOBJ.truename} </FONT> ，你好！ </span>
						<div class="dtree">
			...
			//-->
					...
					<div id="rightFrame">
						<iframe id="rFrame" name="rFrame" width="100%" height="615px"
							...
							scrolling="auto"
							src="${cxp }${not empty defaultLink?defaultLink:'/system/gc.jsp'}"></iframe>
					</div>
				</div> <!-- tree end --></td>
		</tr>
		<tr>
