...
public class Cache implements Cacheable {
    ...
    */
    ...
    public static void main(String[] argv){
    	final Cache cache = new Cache(1500*1024);
    	...
    	es.execute(new Runnable() {
			...
			public void run() {
				while(true){
					...
					cache.add(String.valueOf(temp), new TestObject(temp));
				}
			}
		});
    	es.execute(new Runnable() {
			...
			public void run() {
				while(true){
					cache.get(String.valueOf(random.nextInt(100000)));
				}
			}
		});
    	es.execute(new Runnable() {
			...
			public void run() {
				while(true){
					System.out.println(cache.getCacheHits()*100/(cache.getCacheHits()+cache.getCacheMisses())+"% "+cache.getCacheHits()+" "+cache.getCacheMisses()+" "+cache.getSize()/1024+"k");
					try {
						Thread.sleep(1000l);
...
class TestObject implements Cacheable{
	...
	public TestObject(long test){
		this.test = String.valueOf(test);
	}

	@Override
	public int getSize() {
		return CacheSizes.sizeOfString(test);
	}
