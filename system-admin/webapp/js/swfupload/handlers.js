...
function swfUploadLoaded() {
	...
	document.getElementById(self.customSettings.cancelButtonId).onclick = function () { self.cancelQueue(); document.getElementById(self.customSettings.cancelButtonId).disabled = true;};
	//上传按钮执行的动作 add by stephen
	document.getElementById(self.customSettings.uploadButtonId).onclick = function () { self.startUpload(); };
}
...
function uploadProgress(file,bytesLoaded, bytesTotal) {
	try {
		...
		progress.style.backgroundPosition=(percent-270)+"px 0px";
		
		deleteImg.innerHTML = percent/2+"%";
...
function uploadSuccess(file, serverData) {
	var tR = document.getElementById(file.id);
	if(!tR) return;
	var cells = document.getElementById("cells_"+file.id);
	tR.className += " fileUploading";
	cells.className='flag1';
