...
scheduler._mouse_coords = function (A) {
	...
	if (!this._table_view) {
		...
		B.y = (Math.max(0, Math.ceil(B.x / this._cols[0]) - 1) + Math.max(0, Math.ceil(B.y / this._colsS.height) - 1) * 7) * 24 * 60 / this.config.time_step;
		B.x = 0;
	}
	return B;
};
...
scheduler._reset_scale = function () {
	...
	F = G = this._table_view ? scheduler.date.week_start(L) : L;
...
scheduler._reset_month_scale = function (H, I, G) {
	...
	for (var D = 0; D <= 7; D++) {
		...
		this._colsS[D] = (this._cols[D - 1] || 0) + (this._colsS[D - 1] || 0);
	}
	this._min_date = G;
...
// fliu修改 日期-1天。 
		...
		return scheduler.templates.hour_scale(E) + " - " + scheduler.templates.hour_scale(C);
	}, event_text:function (E, C, D) {
		return D.text;
	}, event_class:function (E, C, D) {
		return "";
	}, event_bar_date:function (E, C, D) {
		return scheduler.templates.hour_scale(E) + " ";
	}, event_bar_text:function (E, C, D) {
		return D.text;
	}};
};
...
scheduler._pre_render_events_table = function (I) {
	...
	for (var C = 0; C < I.length; C++) {
		...
		if (H._sday + H._length <= 7) {
			...
			copy._length = 7 - H._sday;
			copy._eday = 7;
			copy._sday = H._sday;
			copy._sweek = H._sweek;
...
scheduler._copy_event = function (A) {
	return {start_date:A.start_date, end_date:A.end_date, text:A.text, id:A.id};
};
scheduler._rendered = [];
...
scheduler.render_event_bar = function (G) {
	...
	C += scheduler.templates.event_bar_text(G.start_date, G.end_date, G) + "</div>";
	C += "</div>";
	E.innerHTML = C;
...
scheduler._load = function (A, C) {
	...
	if (this._load_mode) {
		...
		dhtmlxAjax.get(A + "&from=" + this._load_format(C) + "&to=" + this._load_format(B), function (D) {
			scheduler.on_load(D);
		});
		this._loaded[this._load_format(C)] = true;
	} else {
		dhtmlxAjax.get(A, function (D) {
			scheduler.on_load(D);
		});
	}
};
...
scheduler._get_lightbox = function () {
	if (!this._lightbox) {
		...
		C[4].innerHTML = scheduler.locale.labels.icon_save;
		C[7].innerHTML = scheduler.locale.labels.icon_cancel;
		C[10].innerHTML = scheduler.locale.labels.icon_delete;
		C[1].innerHTML = B;
...
scheduler.setEventTextStyle = function (B, A) {
	this.for_rendered(B, function (C) {
		C.style.cssText += ";" + A;
	});
