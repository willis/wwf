...
#define NUMBER_ITEMS_ON_LOAD 51
...
- (id)initWithJSON:(id)json
{
    if ((self =[super init]))
    {
        NSArray *list = (NSArray *)[[json objectForKey:@"data"] objectForKey:@"recArticleList"];
        NSLog(@"initWithJSON %@",[[json objectForKey:@"data"] objectForKey:@"recArticleList"]);
        _data = [[NSMutableArray alloc] init];
        NSDictionary *item = nil;
        for (int i = 0; i < list.count; i ++)
        {
            item = [list objectAtIndex:i];
            if ([[item objectForKey:@"picPath"] isEqualToString:@"nopic"]!=TRUE) {
                [_data addObject:[list objectAtIndex:i]];
                NSLog(@"object type : %@",NSStringFromClass([[list objectAtIndex:i] class]));
            }
...
- (void)viewDidLoad
{
    ...
    EGORefreshTableHeaderView *view = [[EGORefreshTableHeaderView alloc] initWithFrame:CGRectMake(0.0f, 30.0f - self.navigationController.view.bounds.size.height, self.view.frame.size.width, self.navigationController.view.bounds.size.height)];

    [self.view addSubview:view];

    self.title = @"网易阅读";
 
    UIImage* titleImage = [UIImage imageNamed:@"logo"];
    ...
    /*
    //设置导航栏背景图片
    //[self.navigationController.navigationBar setBackgroundImage:[UIImage imageNamed:@"navigationBarBackgroundRetro.png.png"] forBarMetrics:UIBarMetricsDefault];

    //导航栏正中央图片
    UIImage * titleImage = [UIImage imageNamed:@"close_x.png"];
    ...
    //绘制导航栏右侧的图片按钮
    UIImage *rightButtonImage = [UIImage imageNamed:@"emblem-important-red.png"];
    ...
    //设置按钮的前景显示图
    [rightButton setImage:[UIImage imageNamed:@"nav-friends-icon.png"] forState:UIControlStateNormal];
    [rightButton setImage:[UIImage imageNamed:@"nav-friends-icon.png"] forState:UIControlStateHighlighted];
    ...
    //绘制导航栏左侧的图片按钮
    UIImage *leftButtonImage = [UIImage imageNamed:@"nav-bar-button.png"];
    UIImage *leftbuttonNormal = [leftButtonImage stretchableImageWithLeftCapWidth:10 topCapHeight:10];
...
- (GMGridViewCell *)GMGridView:(GMGridView *)gridView cellForItemAtIndex:(NSInteger)index
{
    ...
    if (!cell) 
    {
        ...
        view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"close_x.png"]];
    ...
    NSString *url = [[_data objectAtIndex:index] valueForKey:@"picPath"];
    UIImageView *imageView = [[UIImageView alloc] init];
    if ([url isEqualToString:@"nopic"]) {
        imageView.image = [UIImage imageNamed:@"ad"];
        
    }else{
        NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://192.168.0.43:8080/webhtml/vx2/article/%@",(NSString *)[[_data objectAtIndex:index] valueForKey:@"picPath"]]] cachePolicy:NSURLRequestUseProtocolCachePolicy timeoutInterval:30.0];
...
{
    NSObject *object = [_data objectAtIndex:position];
    NSLog(@"Did tap at index:%d value:%@", position,object);
}
...
- (void)infoOptions:(UIBarButtonItem *)barButton
{
    if (INTERFACE_IS_PHONE)
    {
